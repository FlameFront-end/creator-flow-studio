...
import { AssetViewerModal, type AssetViewerPayload } from '../ideas-lab/components/AssetViewerModal'
...
export default function PostDraftExportPage() {
  ...
  const markPublishedMutation = useMutation({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['post-draft-export', id] })
  ...
  const copyCaption = async () => {
    ...
    if (!payload?.caption) {
      showErrorToast(new Error('Нет подписи'), 'Р’ черновике нет подписи для копирования')
      return
    }
