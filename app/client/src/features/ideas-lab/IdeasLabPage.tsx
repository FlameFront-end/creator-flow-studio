...
type IdeasLabWorkspace = 'brief' | 'ideas' | 'logs'

const isIdeasLabWorkspace = (value: string | undefined): value is IdeasLabWorkspace =>
  value === 'brief' || value === 'ideas' || value === 'logs'

const workspaceSteps: {
  value: IdeasLabWorkspace
  label: string
...
export function IdeasLabPage() {
  ...
  const workspace: IdeasLabWorkspace = isIdeasLabWorkspace(workspaceParam)
    ? workspaceParam
    : 'brief'
  ...
  useEffect(() => {
    if (!isIdeasLabWorkspace(workspaceParam)) {
        ...
        <Stack gap="md">
          ...
          <AppTabs
            ...
            onChange={(value) => {
              ...
              if (!isIdeasLabWorkspace(nextWorkspace)) {
                return
              }
              navigate(buildIdeasLabRoute(nextWorkspace))
            }}
          ...
          <Stack gap="sm">
            ...
            {ideasCount > 0 ? (
              <Group justify="flex-end">
                <AppButton buttonVariant="white" onClick={() => navigate(buildIdeasLabRoute('ideas'))}>
                  Перейти к идеям
                </AppButton>
              </Group>
            ) : null}
